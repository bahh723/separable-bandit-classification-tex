\section{Proofs of Theorems~\ref{theorem:polynomial-approximation-1}~and~\ref{theorem:polynomial-approximation-2}}
...
$$
nk - kj + k - nj + j^2 - j \le nk - nj + n - kj + j^2 - j \; .
$$
We cancel common terms and get an equivalent inequality $k \ge n$, which
...
$\norm{\sum_{j=1}^n p_j} \le \sum_{j=1}^n \norm{p_j}$.
Consequently,
...
The first inequality of part 3 follows from triangle inequality in Euclidean spaces, by viewing the polynomials
$p = \sum_{A} c_A x^A$ as multidimensional vectors $(c_A)$, and $\| p \| = \| (c_A) \|$.

For the second inequality, by~\autoref{lemma:simple-inequality}, we have
\[
...
from the book by \citet{Mason-Handscomb-2002}.
...
\end{align*}
We define the polynomial $q:\R^d \to \R$ as
$$
q(x) = \left[ \sum_{i=1}^m A_{s,r}\left( \frac{\ip{v_i}{x}}{\gamma} \right) \prod_{\substack{j ~:~ 1 \le j \le m \\ j \neq i}} B_{s,r} \left( \frac{\ip{v_j}{x}}{\gamma} \right) \right]
- \left(m - \frac{1}{2} \right) \prod_{j=1}^m B_{s,r} \left( \frac{\ip{v_j}{x}}{\gamma} \right) \; .
$$
Finally, we define $p(x) = 2^{-s(s+1)rm+1} q(x)$. We are going to show
that this polynomial $p$ satisfies the required properties.
...
$$
q(x) = Q(x) \prod_{j=1}^m B_{s,r} \left( \frac{\ip{v_j}{x}}{\gamma} \right) \; .
$$
...
$(z+2^{j})^2 \ge (2^j)^2 \ge 4 (z - 2^j)^2$. We can write
$\frac{P_n(z)}{-P_n(-z)}$ as a product of $n+1$ non-negative fractions
$$
\frac{P_n(z)}{-P_n(-z)}
...
\begin{align*}
|p(x)|
& = 2^{-s(s+1)rm+1} \cdot |Q(x)| \prod_{j=1}^m B_{s,r} \left( \frac{\ip{v_j}{x}}{\gamma} \right) \\
& \ge 2^{-s(s+1)rm+1} \cdot |Q(x)| \left( 2^{s(s+1)r} \left( 1 - \frac{1}{4m+1} \right) \right)^m \\
& \ge |Q(x)| \ge \frac{1}{2} \quad \text{(\autoref{lemma:properties-of-q})} \; .
\end{align*}
...
\end{align*}
Finally,
$\norm{p} = 2^{-s(s+1)rm+1} \norm{q} \leq (4m-1) 2^m \cdot \left(2^s rm (4s+2) \right)^{(s+1/2)rm}$. The theorem follows.
