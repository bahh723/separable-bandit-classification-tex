...
\]
Finally, we define the polynomial $p := 2^{-s(s+1)rm+1} \cdot p'$. We are going to show
...
\end{align*}
Finally,
$\norm{p} = 2^{-s(s+1)rm+1} \norm{p'} \leq (4m-1) 2^m \cdot \left(2^s rm (4s+2) \right)^{(s+1/2)rm}$. The theorem follows.
