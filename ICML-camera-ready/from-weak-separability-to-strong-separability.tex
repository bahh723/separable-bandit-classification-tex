...
coordinates of $\ell_2$ by $d$-tuples $(\alpha_1, \dots, \alpha_d)$ of
non-negative integers, the feature map that corresponds to $k$ is $\phi: \B(\zero,1)
\to \ell_2$,
\begin{align}
& \left(\phi(x_1, x_2, \dots, x_d)\right)_{(\alpha_1, \dots, \alpha_d)} \nonumber \\ 
&= x_1^{\alpha_1} x_2^{\alpha_2} \dots x_d^{\alpha_d} \cdot \sqrt{2^{-(\alpha_1 + \dots + \alpha_d)} \binom{\alpha_1 + \dots + \alpha_d}{\alpha_1,  \dots, \alpha_d}}
\label{equation:phi}
\end{align}
where $\binom{\alpha_1 + \dots + \alpha_d}{\alpha_1, \dots,
\alpha_d} = \frac{(\alpha_1 + \dots + \alpha_d)!}{\alpha_1! \dots \alpha_d!}$ is the multinomial coefficient. It can be easily checked that
$$
