...
\begin{lemma}
   For $i=1,\ldots, \ell$ let $q^{(i)}(x)=\sum_S c^{(i)}_{S} x_S $ be a polynomial over $x_1, \ldots, x_d$ with $\|q^{(i)}\|^2\leq M_i$. Then (1) if $q^{(1)}\ldots q^{(\ell)}$ has degree at most $\deg$, we have $\|q^{(1)}\ldots q^{(\ell)}\|^2 \leq (\deg)^{O(\deg)} \prod_i M_i$, and (2) we have $\|q^{(1)}+\ldots+q^{(\ell)}\|^2\leq \ell(M_1+\ldots+M_\ell)$. 
\end{lemma}
\begin{proof}
