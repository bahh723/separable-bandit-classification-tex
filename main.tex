...
$M$ counts the number of different $(S_1,\ldots, S_\ell)$'s with $S_1\times \ldots \times S_\ell=S$. Since $S$ has degree at most $\deg$, we can bound $M$ by $ \ell^{\deg}$. \footnote{This can be seen in two ways. First, we can think of ``assigning'' elements in $S$ to one of $(S_1,\ldots, S_\ell)$; every partition $S_1,\ldots,S_\ell$ can be achieved by some assignment - this can have at most $\ell^{\deg}$ ways. Second, consider $S = (s_1, \ldots, s_d)$.
Consider decomposing each coordinate of $S$ - coordinate $i$ has ${s_i + \ell - 1 \choose \ell - 1}$ ways to partition to $\ell$ parts. Therefore the total number of possible partitions is $\prod_{i=1}^d {s_i + \ell - 1 \choose \ell - 1} \leq \prod_{i=1}^d \ell^{s_i} \leq \ell^{\deg}$.}
...
\For{$t=1, \ldots, T$}{
   ...
   \Else{
       Draw $\tilde{y}_t = \hat{y}_t \triangleq \argmax_{i\in[K]}(W_tx_t)_i$.
   }
   \If{ $\tilde{y}_t=y_t$ }{
       $Z_t\leftarrow 1$, \\
       $M_{t+1}\leftarrow M_t + x_tx_t^\top$,\\
...
\begin{algorithm}[H]
\caption{Confidit \cite{crammer2013multiclass}}
\textbf{Definition}: $\epsilon\triangleq \Delta/(2\gamma_u C')$, where $C'$ is defined in Lemma~\ref{lem:decreasing error}, \\
$\eta\triangleq \frac{\sup_{W\in\Omega}\|W\|_F^2}{\gamma_\ell}$, \\
$\Omega\triangleq$ a convex set where $W^*$ lies in.  \\
$\ell_t(W)=\Phi(Wx_t) - (Wx_t)_{y_t}$.\\
\textbf{Initialization}: $W_1=\mathbf{0}^{K\times d}, M_{1,i}=I^{d\times d}, \forall i\in[K]$.\\
\For{$t=1, \ldots, T$}{
   Observe $x_t$. \\
   Draw $\tilde{y}_t = \argmax_{i\in[K]} \left\{ (W_tx_t)_i + \|x_t\|_{M_{t,i}^{-1}} \right\}$. \\
   $M_{t+1,\tilde{y}_t}\leftarrow M_{t,\tilde{y}_t} + x_tx_t^\top$, $M_{t+1,i}\leftarrow M_{t,i}, \forall i\neq \tilde{y}_t,$\\
   \If{ $\tilde{y}_t=y_t$ }{
       $Z_t\leftarrow 1$, \\
       $M_{t+1,y_t}\leftarrow M_{t,y_t} + x_tx_t^\top$, $M_{t+1,i}\leftarrow M_{t,i}, \forall i\neq y_t,$\\
       $W_{t+1} = \argmin_{W\in\Omega}\left\{ \sum_{s=1}^t Z_s \ell_s(W) + \frac{1}{2\eta}\|W\|_F^2 \right\}$.
   }
   \Else{
       $Z_t\leftarrow 0$,  \\
       $M_{t+1,i}\leftarrow M_{t,i}, \forall i$,\\
       $W_{t+1}\leftarrow W_t$.
   }
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%% !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
%%%% Below are failed attempt to prove the hardness
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\iffalse
