...
regret bound of order $\widetilde{O}(\sqrt{T})$ to $O(T^{3/4})$. In the weakly
separable setting, these algorithms only guarantees a mistake upper bound of
...
the setting of $\alpha$ makes the regret bound $O(\sqrt{T})$, the logistic loss
of the best linear predictor would be $\Omega(T^{3/4})$. Therefore,
\textsc{Newtron} is still only able to guarantee a mistake upper bound of
$O(\sqrt{T})$ in the weakly separable setting.
