...
\FOR{$t=1,2,\ldots,T$:}
\IF{$\min_{(x,y) \in S} \norm{x_t - x} \le \gamma$}
  \STATE{Find nearest neighbor \\ $(\widetilde{x}, \widetilde{y}) = \argmin_{(x,y) \in S} \norm{x_t - x}$}
	\STATE{Predict $\widehat{y}_t = \widetilde{y}$}
\ELSE
	\STATE{Predict $\widehat y_t \sim \text{Uniform}(\{1,2,\dots,K\})$}
	\STATE{Receive feedback $z_t = \indicator{\widehat{y}_t \neq y_t}$}
