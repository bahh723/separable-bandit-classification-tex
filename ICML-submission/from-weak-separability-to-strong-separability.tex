...
%The polynomial separates examples in
%one class from examples in the other classes.
%
%the indicator function corresponding to the intersection of
%$m=K-1$ halfspaces
%a function that takes value $\frac {\gamma'} 2$ on $R_i^+$
%and $-\frac {\gamma'} 2$ on $R_i^-$

\paragraph{Details of the proof.} Before we state the theorems, recall that a polynomial of $d$ variables is a
...
The following lemma establishes a correspondence
