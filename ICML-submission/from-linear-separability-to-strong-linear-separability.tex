...
\gamma_1 & = \frac{1}{2\sqrt{K} \left(376 \lceil \log_2(2m) \rceil \cdot \left \lceil \sqrt{\frac{2}{\gamma}} \right \rceil \right)^{\frac{1}{2} \lceil \log_2(2K-2) \rceil \cdot \left \lceil \sqrt{\frac{2}{\gamma}} \right \rceil}} \\
\gamma_2 & = ??
\end{align*}
...
$$
\norm{\phi(x_t)}_{\ell_2} = K_{\phi}(x_t,x_t) = \frac{1}{1 - \frac{1}{2} \norm{x_t}^2} \le 2 \; .
$$
...
\end{align*}
Futhermore, for all $t=1,2,\dots,T$, if $y_t = i$ then $p_i(x_t) \ge 1/2$, $q_i(x_t) \ge ???$ and if $y_t \neq i$ then $p_i(x_t) \le -1/2$,
$q_i(x) \le - ???$
and
\begin{align*}
...
strongly linearly separable with margin $\gamma_1$ and also strongly separable with
margin $\gamma_2$. Therefore, the examples are strongly separable with margin $\gamma' = \max\{\gamma_1, \gamma_2\}$.
\end{proof}
