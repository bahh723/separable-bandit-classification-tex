...
adversary lie in $\R^d$ and
are \emph{linearly separable with a margin}
(see Definition~\ref{def:linear-sep} for a formal description).
...
\emph{strong linear separability} (see Definition~\ref{def:strong-linear-sep} for a formal description). For binary classification, these two
definitions are identical.
However, they differ for multiclass classification with $K \geq 3$.
