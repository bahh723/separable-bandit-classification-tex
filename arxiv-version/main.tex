...
multivariate polynomials to approximate the indicator function corresponding to
...
values). The parameters are quantified in
\autoref{theorem:polynomial-approximation-1} and
\autoref{theorem:polynomial-approximation-2} stated below.
...
$$
T_n(\cosh(\theta)) \ge 1 + n^2 (\cosh(\theta) - 1) \qquad \text{for any $\theta \ge 0$.}
$$
For $\theta = 0$, according to \eqref{equation:hyperbolic-chebyshev} both sides
are equal to $1$. The proof will be finished, if we prove that the derivative of
the left hand side is greater or equal to the derivative of the right hand side.
That is, we need to prove that
\begin{equation}
\label{equation:hyperbolic-chebyshev-derivative-inequality}
\frac{\diff T_n(\cosh(\theta))}{\diff \theta} \ge \frac{\diff (1 + n^2 (\cosh(\theta) - 1))}{\diff \theta} \qquad \text{for any $\theta \ge 0$.}
\end{equation}
Using \eqref{equation:hyperbolic-chebyshev} and recalling that $\diff \cosh(\theta) / \diff \theta = \sinh(\theta)$ we
