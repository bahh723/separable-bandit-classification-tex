...
\newcommand{\calF}{\mathcal{F}}  % set of real numbers
...
%We focus on the setting where there are $K$ classes, and there is a $d$-dimensional multiclass linear classifier that separates all the examples shown with a positive margin $\gamma$.
%$2^{O(\min(K \log^2 K \log^2 \frac 1 \gamma, \log K \log \log K \sqrt \frac 1 \gamma \log \frac 1\gamma))}$
%$\min( (\log K \sqrt{\frac 1 \gamma})^{O(\log K \sqrt{\frac 1 \gamma})}, K^{O(K \log K \log^2 \frac1\gamma)} )$.
...
%In this problem, in each round, the
%learner receives a feature vector and is asked to predict one of $K$
%possible classes. To allow
...
%round.
%Immediately after the learner makes its prediction, it
...
%does \emph{not} receive the correct label. The feedback is only binary.
...
%where the binary feedback corresponds to a click
%The
...
%\cite{Crammer-Dekel-Keshet-Shalev-Shwartz-Singer-2006}).
