...
\label{theorem:strongly-separable-example-mistake-lower-bound} Let $K$ be a
positive integer, let $\gamma$ be a positive real number and let $R$ be a
non-negative real number. For any algorithm for the \textsc{Online Multiclass
Classification with Bandit Feedback} there exists an inner product space $(V,
...
number mistakes the algorithm is at least $K \lfloor (R/\gamma)^2 \rfloor/ ??$.
\end{theorem}
