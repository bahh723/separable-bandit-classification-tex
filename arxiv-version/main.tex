...
10.10]{Davidson-Donsig-2010}. In our case, we use a multivariate polynomials to
...
The parameters are quantified in \autoref{theorem:polynomial-approximation-1}
and \autoref{theorem:polynomial-approximation-2}
stated below.
...
\begin{theorem}[Polynomial approximation of intersection of halfspaces (1)]
...
\begin{theorem}[Polynomial approximation of intersection of halfspaces (2)]
...
\begin{proposition}
Chebyshev polynomials satisfy for any $n \ge 0$
\begin{enumerate}
\item $|T_n(z)| \le 1$ for all $z \in [-1,1]$.
\item If $z > 1$ then $T_n(z) \ge 1 + n^2(z - 1)$.
\item Absolute value of all coefficients of $T_n(z)$ is at most $2^n$.
\end{enumerate}
\end{proposition}
