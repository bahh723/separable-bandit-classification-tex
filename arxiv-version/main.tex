...
explained in Figure~\ref{figure:pizza-slice}.
...
$$
p(x) = \sum_{i=1}^m A_{s,r}\left( \frac{\ip{v_i}{x}}{\gamma} \right) \prod_{\substack{j ~:~ 1 \le j \le m \\ j \neq i}} B_{s,r} \left( \frac{\ip{v_i}{x}}{\gamma} \right)
- \left(m - \frac{1}{2} \right) \prod_{j=1}^m B_{s,r} \left( \frac{\ip{v_i}{x}}{\gamma} \right) \; .
$$
...
$$
p(x) = Q(x) \prod_{i=1}^m B_{s,r} \left( \frac{\ip{v_i}{x}}{\gamma} \right) \; .
$$
...
$z \in [0,1]$. If $z=1$ then $P_n(z)=P_n(-z) = 0$. For $z \in [0,1)$, we have
$$
\frac{P_n(-z)}{P_n(z)} = \frac{1+x}{1-x} \prod_{i=1}^n \frac{(x+2^i)^2}{(x-2^i)^2} \ge 1 \; .
$$
TODO
\end{proof}
