...
algorithm receives a feature vector and algorithm has to predict one of $K$
possible classes. Imediately after the algorithm makes its prediction, it
...
prove that any algorithm makes $K \lfloor (R/\gamma)^2 \rfloor / ??$ in the
worst case in this setting.
...
Bandit Feedback} with $K \le \frac{1}{2}(R/\gamma)^2$ classes there exists an inner
...
\le R$ and the expected number of mistakes the algorithm makes is at least $K
\lfloor \frac{1}{8} (R/\gamma)^2 \rfloor/ ??$.
\end{theorem}

\begin{proof}
Let $M = \lfloor \frac{1}{8} (R/\gamma)^2 \rfloor$. Let $V = \R^{M+1}$ equipped with the
standard inner product.  Let $e_1, e_2, \dots, e_{M+1}$ be the standard
orthonormal basis of $\R^{M+1}$. We define vectors $v_1, v_2, \dots, v_M \in V$
where $v_i = \frac{R}{\sqrt{2}}(e_i + e_{M+1})$ for $i=1,2,\dots,M$. Let $\ell_1, \ell_2,
\dots, \ell_M$ be chosen i.i.d. uniformly at random from $\{1,2,\dots,K\}$ and
independently of any randomness used the by the algorithm.

Let $T = M K$. We define examples $(x_1, y_1), (x_2, y_2), \dots, (x_T, y_T)$ as
follows:
$$
...
Clearly, norm of each example is $R$. We show that the examples are strongly
separable with margin $\gamma$. In order to see that consider $w_1^*, w_2^*,
\dots, w_K^* \in V$ defined by
$$
w_i^* = 2 \frac{\gamma}{R} \left( \sum_{j ~:~ \ell_j = i} e_j \right) - \frac{\gamma}{R} e_{M+1} \qquad \quad \text{for $i=1,2,\dots,K$.}
$$
...
$$
\sum_{i=1}^K \norm{w_i^*}^2
= 4 \frac{\gamma^2}{R^2} \sum_{j=1}^M \norm{e_j}^2 + \frac{\gamma^2}{R^2} K \norm{e_{M+1}}^2
= 4\frac{\gamma^2}{R^2} M + \frac{\gamma^2}{R^2} K
\le \frac{1}{2} + \frac{1}{2}
= 1 \; .
$$

It remains to show that the expected number of mistakes the algorithm makes is
at least $K M / ??$.
