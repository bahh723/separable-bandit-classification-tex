...
where function
\[
S_n^k(x) = \frac{P_n(-x)^{h(k)} - P_n(x)^{h(k)}}{P_n(-x)^{h(k)} + P_n(x)^{h(k)}} = \frac{A_n^k(x)}{B_n^k(x)},
\]
...
\begin{enumerate}
\item If $x$ is in $[1, 2^n]$, then $S_n(x)$ is in $[1, 1 + \frac 1 k]$.
\item If $x$ is in $[2^{-n}, -1]$, then $S_n(x)$ is in $[-1 - \frac 1 k, -1]$.
\item If $x$ is in $[-1, 1]$, then $|S_n(x)|$ is in $[-1, 1]$.
\item For all $x$ in $\fR$, $|B_n^k(x)| \geq \frac{2k}{2k+1} 2^{\frac{n(n-1)h(k)}2}$.
\end{enumerate}
\end{fact}
