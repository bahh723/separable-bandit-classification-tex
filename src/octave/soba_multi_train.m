function model = soba_multi_train(X,Y,model)
...
for i=1:n
    ...
    %model.gamma=min(.1*sqrt(model.n_cla*(1+model.sum_q)/model.iter),1);
    model.gamma=0.01;
    ...
    if y_tilde==Yi
      if y_tilde~=y_hat
        ...
        q=sqrt(Prob(y_tilde))*model.invA*tmp(:);
        res=sqrt(Prob(y_tilde))*tmp(:)'*q;
        ...
        tmp=spalloc(model.n_cla,size(X,1),2*size(X,1));
        tmp(y_hat2,:) = -1/Prob(y_tilde)*X(:,i)';
        tmp(Yi,:) = 1/Prob(y_tilde)*X(:,i)';
        q=model.invA*(sqrt(Prob(y_tilde))*tmp(:));
        res=sqrt(Prob(y_tilde))*(tmp(:)'*q);
        diff=mx_f2-mx_f;
        m=(diff^2/(1+res)+2*diff/(1+res))/Prob(y_tilde);
        %if m+model.sum_m>=-sqrt(model.iter)
        %if m+model.sum_m>=0
        if 1
          model.theta=model.theta+tmp;
          model.invA=model.invA-(q/(1+res))*q';
          model.w=reshape(model.invA*model.theta(:),model.n_cla,size(X,1));
    ...
    if mod(i,model.step)==0
      fprintf('#%.0f AER:%5.2f Log term:%f MarginUpdates:%.0f Sum M: %f\n', ...
            ceil(i/1000),model.aer(model.iter)*100, model.sum_q, model.mupd, model.sum_m);
            %fflush(stdout);
    end
end
